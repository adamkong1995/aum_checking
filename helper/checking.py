...
def Checking(filePath, startdate):
    ...
    aum = aum.style.applymap(check_3_pct,subset=['nav_chg', 'navps_chg', 'shrs_chg', 'mf_chg', 'pf_chg']).format({'nav':"{:.2f}", 'navps':"{:.2f}", 'commitment': "{:.2f}", 'subscription': "{:.2f}", 'redemption': "{:.2f}", 'distribution': "{:.2f}", 'committed': "{:.2f}"})

    return aum

def download_excel():
    timestr = time.strftime("%Y%m%d-%H%M%S")
    excel = aum.to_excel('result %s.xlsx' % (timestr))
    return excel
